...
class NotificationsRemoteDataSourceImpl implements NotificationsRemoteDataSource {
  ...
  Future<ApiResponse<void>> deleteAllNotifications() async {
    try {
      final response = await dioService.post(ApiEndpoints.deleteAllNotifications);

      if (response.statusCode == 200) {
        return ApiResponse.success(
          data: null,
          message: response.data['message'] ?? 'تم حذف جميع الإشعارات بنجاح',
        );
      } else {
        return ApiResponse.error(
          message: response.data['message'] ?? 'فشل في حذف الإشعارات، يرجى المحاولة مرة أخرى',
          statusCode: response.statusCode,
        );
      }
    } catch (e) {
